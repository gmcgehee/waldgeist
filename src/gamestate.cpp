...
Move GameState::localConvertToMove(Square destination, Square origin, PieceType promotion_piece, SpecialMoveFlag flag) const
{
...
void GameState::resetBoardState()
{
...
void GameState::loadFromFen(std::string const fen)
{
...
Bitboard GameState::getSideState(Side side) const
{
...
Bitboard GameState::getFullState() const
{
...
void GameState::setPieceAt(Square square, Piece piece)
{
    mailbox[square] = piece;
    *piece.piece_bb |= 1ULL << square;
}

void GameState::unsetPieceAt(Square square)
{
...
bool GameState::isSquareThreatened(Square square, Side bySide) const
{
...
bool GameState::make(Move move, Undo &undo)
{
...
void GameState::unmake(Move move, Undo undo)
{
...
void GameState::make_null()
{
    state.sideToPlay = state.sideToPlay == WHITE ? BLACK : WHITE;
    state.enPassantSquare = OUT_OF_BOUNDS;
}

void GameState::unmake_null(Square original_en_passant_square)
{
    state.sideToPlay = state.sideToPlay == WHITE ? BLACK : WHITE;
    state.enPassantSquare = original_en_passant_square;
}

void GameState::UCI_make(std::string str_move)
{
    Undo temp_undo;
...
Move GameState::UCI_convert_to_move(std::string str_move) const
{
